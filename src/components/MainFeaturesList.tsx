...
import { Search, Layers, BookOpen, Archive, PlusCircle, Folder, CheckCircle } from 'lucide-react';
...
export default function MainFeaturesList() {
  ...
  return (
    <section className="space-y-12 py-16 border-t border-slate-100" id="platform-key-features">
      <div className="text-center space-y-4">
        ...
        <p className="text-slate-500 text-xs md:text-sm font-bold uppercase tracking-tight max-w-2xl mx-auto">
          Delivering a reliable, highly indexed, and community-driven resource index for university schedules.
        </p>
      </div>
        ...
        {features.map((feat, idx) => {
          ...
          return (
            <motion.div
              ...
              className="p-5 sm:p-6 lg:p-8 bg-white border border-slate-200/80 rounded-apple-2xl shadow-sm hover:border-emerald-200 hover:bg-emerald-50/30 transition-all flex flex-col justify-between hover:shadow-md group"
              id={feat.id}
            >
              <div className="space-y-6">
                <div className="w-12 h-12 bg-white border border-emerald-100 text-emerald-600 flex items-center justify-center rounded-apple shadow-emerald-sm group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300">
                  <Icon size={20} />
                </div>
