...
export default function AIFeatures() {
  ...
  const [particle, setParticle] = useState<
    "Electron" | "Proton" | "Muon" | "Alpha Particle"
  >("Electron");
  const [viewType, setViewType] = useState<"wavefunction" | "probability">(
    "probability",
  );
  ...
  const [studentContext, setStudentContext] = useState<string>(
    "B.Sc. Physics (Hons) Undergraduate student",
  );
  ...
  const [trajectory, setTrajectory] = useState<
    { x: number; y: number; r: number; phi: number }[]
  >([]);
  const [grOutcome, setGrOutcome] = useState<
    "CROSS_HORIZON" | "ESCAPED" | "STABLE_ORBIT" | "MAX_BOUND"
  >("STABLE_ORBIT");
  ...
  const calculateTrajectory = (
    ...
    const dv_dlambda = (currR: number, currVr: number) => {
      ...
      const termRelativistic =
        (3.0 * M * LVal * LVal) / (currR * currR * currR * currR);
      return termCentrifugal - termNewtonian - termRelativistic;
    };

    let outcome: "CROSS_HORIZON" | "ESCAPED" | "STABLE_ORBIT" | "MAX_BOUND" =
      "MAX_BOUND";
    ...
    for (let step = 0; step < maxSteps; step++) {
      ...
      const nextR =
        r + (dlambda / 6.0) * (k1_r + 2.0 * k2_r + 2.0 * k3_r + k4_r);
      const nextVr =
        vr + (dlambda / 6.0) * (k1_vr + 2.0 * k2_vr + 2.0 * k3_vr + k4_vr);
  ...
  useEffect(() => {
    const { points, outcome } = calculateTrajectory(
      isPhoton,
      grR0,
      grL,
      grVr0,
  ...
  const handleAnalyzeGeodesic = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          "Einstein relativity cluster analytical request failed under load.",
        );
      }
      ...
      console.error(err);
      setRelativityError(
        err.message || "Failed to compile Schwarzschild geodesic analysis.",
      );
    } finally {
  ...
  useEffect(() => {
    ...
    const energy =
      (baseConstant * Math.pow(quantumN, 2)) / Math.pow(wellWidth, 2);
    if (energy < 0.001) {
  ...
  const handleAISynergize = async () => {
    ...
    try {
      ...
      setSimError(
        e.message || "An unexpected simulation service error occurred.",
      );
    } finally {
      setSimLoading(false);
    }
  ...
  const uploadBinaryFile = async (file: File) => {
    ...
    try {
      ...
      setDiagError(
        e.message || "Failed to upload file to the backend analyzer.",
      );
    } finally {
  ...
  const handleRunDiagnostics = async () => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error("Academic Advisor Agent reached resource exhaustion.");
      const data = await res.json();
      if (data.error) throw new Error(data.error);
      ...
      setDiagLogs((prev) => [
        ...prev,
        "Diagnostics generation complete. Formatting dashboards.",
      ]);
      ...
      setDiagError(
        e.message ||
          "An unexpected engine failure occurred during computation.",
      );
    } finally {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto py-10 md:py-16 px-4 space-y-12">
      ...
      <div className="text-center space-y-4 max-w-3xl mx-auto">
        <div className="w-16 h-16 bg-emerald-50 text-emerald-600 rounded-[1.5rem] mx-auto flex items-center justify-center border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-emerald-100 shadow-sm">
          <Atom size={32} strokeWidth={1.5} className="text-emerald-600" />
        </div>
        <h1 className="text-3xl md:text-5xl font-black tracking-tight text-slate-900">
          DeepMind Physics & Analytics Lab
        </h1>
        <p className="text-slate-500 font-medium text-base md:text-lg leading-relaxed">
          Advanced computational simulations, quantum well solvers, and
          automated student scorecard diagnostics agents.
        </p>
      ...
      <AnimatePresence>
        {activeSegment === "sim" && (
          ...
          >
            {/* SETUP PARAMETERS */}
            <div className="lg:col-span-4 bg-white border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-200 p-6 rounded-[2rem] shadow-sm space-y-8">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 block">
                    Select Quantum Particle
                  </label>
                  <div className="grid grid-cols-2 gap-2">
                    {(
                      ["Electron", "Proton", "Muon", "Alpha Particle"] as const
                    ).map((p) => (
                      <button
                        key={p}
                        onClick={() => setParticle(p)}
                        className={`py-2 px-3 text-xs font-bold rounded-xl border-y border-x-0 sm:border sm:border-x transition-all text-left ${
                          particle === p
                ...
                <div className="space-y-2">
                  ...
                  <span className="text-[10px] text-slate-400 block leading-tight">
                    Controls wave oscillation nodes & energy eigenvalue limits.
                  </span>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  />
                  <span className="text-[10px] text-slate-400 block leading-tight">
                    Width compression directly scales kinetic energy potential
                    limits.
                  </span>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-200 p-1 rounded-2xl bg-slate-50">
                    <button
              ...
              <div className="bg-slate-50 rounded-2xl p-4 border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-100 space-y-3">
                ...
                <p className="text-[10px] text-slate-500 leading-relaxed">
                  Formula:
                  <br />
                  <code className="font-mono bg-white p-1 rounded border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-205 block text-center text-[10px] mt-1 text-slate-700">
                    E_n = (n² * h²) / (8 * m * L²)
                  </code>
                </p>
              </div>
            ...
            <div className="lg:col-span-8 space-y-8">
              <div className="bg-white border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-200 p-6 md:p-8 rounded-[2rem] shadow-sm space-y-6">
                <div className="flex flex-wrap justify-between items-center gap-4">
                  <div className="space-y-1">
                    <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 text-emerald-800 border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-emerald-100 rounded-full text-[10px] font-black uppercase tracking-wider">
                ...
                {/* SVG WAVE CHART */}
                <div className="relative border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-100 rounded-2xl bg-slate-50 overflow-hidden py-4">
                  <svg
                    viewBox="0 0 600 240"
                  ...
                  >
                    ...
                    {viewType === "wavefunction" ? (
                      ...
                      <>
                        <polygon
                          points={`40,200 ${generateSvgPath()} 560,200`}
                          fill="#e6f4ea"
                          opacity="0.65"
                        />
                        <path
                  ...
                  <div className="absolute top-4 left-6 text-[10px] font-mono text-slate-400 space-y-1">
                    <div>
                      Ψ_max = √(2/L) = {Math.sqrt(2 / wellWidth).toFixed(3)}
                    </div>
                    <div>
                      λ_n = 2L/n = {((2 * wellWidth) / quantumN).toFixed(2)} Å
                    </div>
                  </div>
                </div>

                {/* AI Theoretical report trigger */}
                <div className="flex flex-col md:flex-row items-center md:justify-between p-6 bg-emerald-50 border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-emerald-100/50 rounded-2xl gap-4">
                  <div className="text-center md:text-left space-y-1">
                    ...
                    <p className="text-xs text-emerald-700/80 max-w-lg">
                      Submit these parameters to generate a profound Bohr
                      correspondence wave theory report.
                    </p>
                  </div>
                ...
                <AnimatePresence>
                  {simError && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      className="p-4 bg-rose-50 border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-rose-100 rounded-xl flex items-start gap-3 mt-4 text-xs font-semibold text-rose-800"
                    >
                      <AlertCircle size={18} className="shrink-0 mt-0.5" />
                      <div>
                        <h5 className="font-extrabold uppercase mb-1">
                  ...
                  {simResult && (
                    <motion.div
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      className="bg-slate-50 border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-200/80 rounded-2xl p-6 space-y-6"
                    >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-1">
                          ...
                          <div className="bg-white border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-100 p-3 rounded-xl text-center font-mono text-xs font-bold text-emerald-800">
        ...
        {activeSegment === "relativity" && (
          ...
          >
            ...
            <div className="lg:col-span-4 bg-white border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-200 p-6 md:p-8 rounded-[2rem] shadow-sm space-y-8">
              <div className="flex items-center gap-2.5 pb-4 border-b border-slate-100">
                <Sliders size={20} className="text-emerald-600" />
                <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wider">
                  Einstein Orbit Parameters
                </h3>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className={`py-2.5 px-3 text-[10px] font-black uppercase rounded-xl border-y border-x-0 sm:border sm:border-x transition-all text-center cursor-pointer ${
                        !isPhoton
                          ? "border-emerald-600 bg-emerald-50 text-emerald-800 font-bold"
                          : "border-slate-200 text-slate-600 hover:bg-slate-50"
                      }`}
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-xs font-bold uppercase tracking-wider text-slate-500">
                    <span className="text-[10px] font-black uppercase text-slate-400 font-sans">
                      Angular Momentum (L)
                    </span>
                    <span className="font-mono text-emerald-700 font-extrabold text-[11px]">
                      {grL.toFixed(2)}
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center text-xs font-bold uppercase tracking-wider text-slate-500">
                    <span className="text-[10px] font-black uppercase text-slate-400 font-sans">
                      Integrator Step bounds (dλ)
                    </span>
                    <span className="font-mono text-emerald-700 font-extrabold text-[11px]">
                      {simStepRate.toFixed(3)}
                    </span>
                  </div>
                  <input
                    type="range"
            ...
            <div className="lg:col-span-8 space-y-6">
              <div className="bg-white border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-200 p-6 md:p-8 rounded-[2rem] shadow-sm grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
                {/* HTML5 Dynamic Graphics System */}
                <div className="md:col-span-7 flex flex-col items-center justify-center">
                  <div className="p-1 bg-slate-950 rounded-[1.75rem] border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-800 shadow-inner relative max-w-full overflow-hidden">
                    <canvas
                      ref={grCanvasRef}
                      width={400}
                      height={400}
                    ...
                    />
                    <div className="absolute top-4 left-4 bg-slate-900/80 px-2.5 py-1 border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-800/80 rounded font-mono text-[8px] text-slate-400 font-bold uppercase tracking-wider">
                  ...
                  <div className="space-y-4">
                    ...
                    <h2 className="text-xl font-extrabold uppercase text-slate-900 tracking-tight leading-tight">
                      {grOutcome === "CROSS_HORIZON"
                        ? "Singularity Capture Crossing"
                        : grOutcome === "ESCAPED"
                          ? "Asymptotic Spatial Escape"
                          : grOutcome === "STABLE_ORBIT"
                            ? "Resonant Precession Bound"
                            : "Complex Relativistic Path"}
                    </h2>
                    <p className="text-[11px] text-slate-500 font-semibold uppercase tracking-wide leading-relaxed">
                      Numerical trajectory integrated in real-time. Einsteinian
                      corrections show periapsis precession which is absent in
                      classical models.
                    </p>
                  </div>

                  <div className="border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-150 rounded-2xl divide-y divide-slate-150 overflow-hidden bg-slate-50/50">
                    <div className="p-4 flex justify-between items-center text-xs font-bold uppercase text-slate-800">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded text-[8.5px] font-black border-y border-x-0 sm:border sm:border-x uppercase tracking-wider ${
                          grOutcome === "CROSS_HORIZON"
                    ...
                    <div className="p-4 flex justify-between items-center text-xs font-bold uppercase text-slate-800">
                      ...
                      <span className="font-mono font-black text-slate-900 text-[11px]">
                        {trajectory.length} RK4 Steps
                      </span>
                    </div>

                    <div className="p-4 flex justify-between items-center text-xs font-bold uppercase text-slate-800">
                      <span className="text-[9px] text-slate-400 font-bold tracking-widest uppercase font-mono">
                        Closest Approach Min(r)
                      </span>
                      <span className="font-mono font-black text-slate-900 text-[11px]">
                        {trajectory.length > 0
                    ...
                    <div className="p-4 flex justify-between items-center text-xs font-bold uppercase text-slate-800">
                      ...
                      <span className="font-mono font-black text-slate-900 text-[11px]">
                        {trajectory.length > 0
                          ? (
                              (trajectory[trajectory.length - 1].phi /
                                Math.PI) *
                              180.0
                            ).toFixed(1)
                          : "0.0"}
              ...
              {relativityError && (
                <div className="p-4 bg-red-50 text-red-700 border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-red-150 rounded-2xl text-xs font-bold uppercase tracking-wider text-left">
                  {relativityError}
                </div>
              )}
              ...
              <AnimatePresence>
                {relativityResult && (
                  <motion.div
                    ...
                    transition={{ duration: 0.3 }}
                    className="p-6 md:p-8 bg-slate-950 border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-800 rounded-[2rem] text-left space-y-8"
                  >
                    <div className="flex justify-between items-start flex-wrap gap-4 border-b border-slate-805 border-slate-800 pb-5 text-left">
                      <div className="space-y-1.5">
                        ...
                        <p className="text-[9.5px] text-slate-400 font-bold uppercase tracking-widest">
                          Report Compiled under Einstein Field Equations solver
                          matrices (G_μν = 8πT_μν)
                        </p>
                      </div>
                      <div className="px-3 py-1 bg-emerald-500/10 text-emerald-400 font-black text-[9px] uppercase tracking-widest border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-emerald-500/20 rounded font-mono">
                        VERIFIED BLUEPRINT
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
                      ...
                      <div className="space-y-3">
                        <span className="text-[9px] font-black text-emerald-400 uppercase tracking-widest font-mono block">
                          Effective Potential (V_eff) Interaction
                        </span>
                        <p className="text-xs text-slate-300 leading-relaxed font-semibold uppercase tracking-wider font-sans">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-8 pt-6 border-t border-slate-800 text-left">
                      ...
                      <div className="md:col-span-7 space-y-3">
                        <span className="text-[9px] font-black text-emerald-400 uppercase tracking-widest font-mono block">
                          Relativistic Phenomena Isolated
                        </span>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {(relativityResult.phenomenaObserved || []).map(
                            (phen: string, pIdx: number) => (
                              <div
                                key={pIdx}
                                className="p-3 bg-slate-905 bg-slate-900 border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-800 rounded-xl text-[9px] font-black text-slate-200 uppercase tracking-widest flex items-center gap-2"
                              >
                      ...
                      <div className="md:col-span-5 space-y-3">
                        <span className="text-[9px] font-black text-emerald-400 uppercase tracking-widest font-mono block">
                          Astrophysical Precedents
                        </span>
                        <p className="text-[11px] text-slate-400 leading-relaxed font-bold uppercase tracking-widest bg-slate-900/50 p-4 border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-800 rounded-xl font-sans">
                    ...
                    <div className="pt-4 border-t border-slate-800 text-center font-mono text-[8.5px] text-slate-500 uppercase tracking-wider">
                      Academic Geodesic Monograph Model G-2026 // DeepMind
                      Physics Labs Server Hub
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
        ...
        {activeSegment === "diagnostics" && (
          ...
          >
            ...
            <div className="bg-white border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-200 p-6 md:p-8 rounded-[2rem] shadow-sm space-y-8">
              <div className="flex justify-between items-center flex-wrap gap-4 pb-4 border-b border-slate-100">
                <div className="space-y-1">
                  <h3 className="font-bold text-slate-800 text-lg uppercase tracking-wider">
                    Scorecard Analysis Interface
                  </h3>
                  <p className="text-xs text-slate-400 font-medium">
                    Input, drag & drop your semester transcripts, or choose an
                    academic prototype below.
                  </p>
                </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="py-1.5 px-3 bg-slate-50 border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-200 text-[10px] font-bold text-slate-600 uppercase tracking-wider hover:bg-emerald-50 hover:text-emerald-800 hover:border-emerald-250 rounded-lg transition-all"
                  >
                  ...
                  >
                    ...
                    {fileUploading && (
                      <div className="absolute inset-x-0 bottom-0 top-0 bg-white/95 backdrop-blur-xs flex flex-col items-center justify-center rounded-2xl z-20 space-y-3">
                        ...
                        />
                        <span className="text-xs font-black uppercase tracking-wider text-slate-700 animate-pulse">
                          Uploading file stream. Prepare OCR models...
                        </span>
                        <span className="text-[10px] text-slate-400 font-bold uppercase">
                          Connecting securely to Sandbox File Server
                        </span>
                      </div>
                    )}
                    ...
                    {uploadedFileInfo ? (
                      <div className="flex-1 flex flex-col items-center justify-center text-center p-8 space-y-4">
                        <div className="w-12 h-12 bg-emerald-50 text-emerald-600 rounded-2xl flex items-center justify-center border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-emerald-100 shadow-sm">
                          <FileText size={24} />
                        </div>
                        <div className="space-y-1">
                          ...
                          <p className="text-[10px] text-slate-400 font-bold uppercase">
                            Size:{" "}
                            {(uploadedFileInfo.size / 1024 / 1024).toFixed(2)}{" "}
                            MB • PDF/OCR Document Locked
                          </p>
                        </div>
                        <p className="text-[10px] text-slate-500 font-medium max-w-sm">
                          This document has been uploaded and stored on the
                          Secure Sandbox. Click "Run Diagnostic Agent Engine" to
                          perform OCR and analysis on the backend.
                        </p>
                        <button
                          type="button"
                          onClick={() => {
                          ...
                          className="py-1 px-3 bg-rose-50 hover:bg-rose-100 border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-rose-100 text-[10px] font-bold text-rose-700 uppercase rounded-lg transition-all"
                        >
                          Remove file
                        </button>
                      </div>
                    ...
                    <div className="flex justify-between items-center border-t border-slate-200/60 pt-4 mt-4 flex-wrap gap-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                          Drag/Drop or Paste standard text/PDF/Image transcript
                        </span>
                      </div>

                      <button
                        onClick={() => fileInputRef.current?.click()}
                        className="py-1.5 px-3 text-[10px] bg-white border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-200 font-bold uppercase tracking-wider text-slate-605 hover:bg-slate-50 rounded-lg transition-all flex items-center gap-1.5"
                      >
                        <Upload size={12} />
                        Browse Files
                      </button>
                ...
                <div className="lg:col-span-4 space-y-6">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full text-xs font-bold border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-200 bg-slate-50 p-3.5 rounded-xl outline-none focus:border-emerald-600 transition-all text-slate-700 uppercase tracking-wide"
                    />
                  </div>
                  ...
                  <button
                    ...
                    disabled={
                      diagLoading || (!rawText.trim() && !uploadedFileInfo)
                    }
                  ...
                  >
                    <RefreshCcw
                      size={14}
                      className={diagLoading ? "animate-spin" : ""}
                    />
            ...
            <AnimatePresence>
              {diagError && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0 }}
                  className="p-5 bg-rose-50 border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-rose-100 rounded-[1.5rem] flex items-start gap-3 text-xs font-bold text-rose-800 max-w-4xl mx-auto"
                >
              ...
              {diagLoading && (
                <motion.div
                  ...
                  exit={{ opacity: 0 }}
                  className="bg-slate-900 text-slate-100 p-8 rounded-[2rem] shadow-xl border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-800 max-w-2xl mx-auto space-y-6 flex flex-col items-center justify-center text-center py-12"
                >
                  {/* Custom Physics Matrix Spin Orbit animation */}
                  <div className="relative w-20 h-20 mb-4 flex items-center justify-center">
                    <div
                      className="absolute inset-0 rounded-full border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-dashed border-emerald-500/20 animate-spin"
                      style={{ animationDuration: "6s" }}
                    />
                    <div className="absolute w-16 h-16 rounded-full border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-emerald-500/30 animate-reverse-spin" />
                    <div
                      className="absolute w-10 h-10 rounded-full border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-emerald-400 animate-spin"
                  ...
                  <div className="space-y-2">
                    <h4 className="text-lg font-black text-white uppercase tracking-widest animate-pulse">
                      Running Diagnostic Core
                    </h4>
                    <p className="text-xs text-slate-450 uppercase tracking-wider font-semibold max-w-sm">
                      Please wait while the DeepMind intelligence agent
                      deciphers your transcript score indexes and compiles
                      statistical guidelines.
                    </p>
                  </div>

                  {/* Progress loading steps stack */}
                  <div className="w-full max-w-md bg-white border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-200/80 p-4 rounded-xl text-left text-xs space-y-3 text-slate-700 shadow-sm">
                    {diagLogs.map((log, idx) => (
                      <div
                        key={idx}
                        className="flex gap-3 items-center bg-slate-50 p-2.5 rounded-lg border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-100"
                      >
                        {idx === diagLogs.length - 1 ? (
                          <Loader2
                            size={14}
              ...
              {diagResult && !diagLoading && (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-white border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-200 p-6 rounded-[2rem] shadow-sm flex flex-col justify-between space-y-4">
                      <div>
                        <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block mb-1">
                          AGGREGATE STANDING
                        </span>
                        <h4 className="text-2xl font-black text-slate-800 uppercase tracking-tight">
                          {diagResult.academicStanding}
                        </h4>
                      </div>
                    ...
                    <div className="bg-white border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-200 p-6 rounded-[2rem] shadow-sm flex flex-col justify-between space-y-4 col-span-1 md:col-span-1">
                      <div>
                        <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block mb-1">
                          Primary Discipline
                        </span>
                        <h4 className="text-base font-bold text-slate-800 uppercase tracking-wider truncate">
                          {studentContext}
                        </h4>
                      </div>
                      <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                  ...
                  <div className="bg-white border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-200 p-6 md:p-8 rounded-[2rem] shadow-sm space-y-6">
                    <div className="space-y-1">
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-emerald-50 text-emerald-800 border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-emerald-100 rounded-full text-[10px] font-black uppercase tracking-wider">
                        <FileText size={12} strokeWidth={2} /> Quantitative
                        Scoring Distribution
                      </span>
                      <h3 className="text-xl font-bold text-slate-850">
                        Scholastic Matrix Plot
                      </h3>
                    </div>
                  ...
                  <div className="bg-slate-50 border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-100 p-8 rounded-[2rem] shadow-sm space-y-4">
                    <span className="text-[10px] font-black tracking-widest uppercase text-slate-400">
                      DeepMind Scholastic Overview
                    </span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="bg-white border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-200 p-6 md:p-8 rounded-[2rem] shadow-sm space-y-6">
                      <h4 className="text-sm font-black uppercase tracking-widest text-emerald-850 flex items-center gap-2">
                        ...
                        Key Physics & Mathematical Strengths
                      </h4>
                      <ul className="space-y-3 pl-2">
                        {(diagResult.strengths || []).map((str, idx) => (
                          <li
                            key={idx}
                            className="flex gap-2.5 items-start text-xs text-slate-700 font-semibold leading-relaxed"
                          >
                    ...
                    <div className="bg-white border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-200 p-6 md:p-8 rounded-[2rem] shadow-sm space-y-6">
                      <h4 className="text-sm font-black uppercase tracking-widest text-slate-800 flex items-center gap-2">
                        ...
                        Targeted Focus Areas For Improvement
                      </h4>
                      <ul className="space-y-3 pl-2">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="lg:col-span-7 bg-white border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-200 p-6 md:p-8 rounded-[2rem] shadow-sm space-y-6">
                      <h4 className="text-sm font-black uppercase tracking-widest text-slate-800 flex items-center gap-2">
                        <BookOpen size={16} className="text-emerald-600" />{" "}
                        Prescribed Study Directives
                      </h4>
                      ...
                      <div className="space-y-4">
                        {(diagResult.suggestedActions || []).map((act, idx) => (
                          <div
                            key={idx}
                            className="p-4 bg-slate-50 rounded-2xl border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-100 flex gap-3.5"
                          >
                            ...
                            <div className="space-y-1">
                              <h5 className="text-xs font-black uppercase text-slate-800 tracking-wider leading-snug">
                                {act.title}
                              </h5>
                    ...
                    <div className="lg:col-span-5 bg-white border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-200 p-6 md:p-8 rounded-[2rem] shadow-sm space-y-6">
                      <h4 className="text-sm font-black uppercase tracking-widest text-slate-800 flex items-center gap-2">
                        <Compass
                          size={16}
                        ...
                        />{" "}
                        Future Physics Research Prospects
                      </h4>

                      <div className="space-y-3">
                        {(diagResult.recommendedResearchSubfields || []).map(
                          (sub, idx) => (
                            <div
                              key={idx}
                              className="p-4 border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-150 rounded-2xl flex items-center justify-between text-xs font-bold uppercase text-slate-800 bg-emerald-50/20"
                            >
                              <span>{sub}</span>
                              <ArrowRight
                                size={14}
                      ...
                      <div className="bg-slate-50 rounded-2xl p-4 text-[10px] text-slate-505 font-bold leading-normal uppercase tracking-wider text-center border-y sm:border-y border-x-0 sm:border sm:border-x sm:border-x border-slate-100">
