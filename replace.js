...
// 2. The title
content = content.replace(/text-3xl sm:text-4xl md:text-5xl font-black tracking-tighter text-slate-900 leading-none uppercase/g, 'text-2xl sm:text-4xl md:text-5xl font-black tracking-tighter uppercase text-slate-900 leading-none');

// 3. Tab select class
content = content.replace(/\? "bg-slate-900 text-slate-50 shadow-sm translate-x-1 translate-y-1"/g, '? "bg-emerald-50 text-emerald-600 border border-emerald-200 rounded-apple-xl shadow-sm"');
content = content.replace(/: "bg-white text-slate-900 shadow-sm hover:shadow-none hover:translate-x-1 hover:translate-y-1"/g, ': "bg-white text-slate-900 hover:border-emerald-600 border border-slate-100 rounded-apple-xl shadow-sm hover:bg-emerald-50 transition-all"');
content = content.replace(/bg-slate-900 text-white font-black/g, 'bg-emerald-600 text-white font-black border-emerald-600');
content = content.replace(/bg-slate-800 text-white/g, 'bg-emerald-600 text-white border-emerald-600');
content = content.replace(/bg-slate-900 text-\[\#fcfbf9\]/g, 'bg-emerald-50 text-emerald-700 font-bold border-none');

// 4. Primary Run Button
content = content.replace(/bg-slate-900 hover:bg-slate-800 disabled:opacity-50 text-white font-black text-xs uppercase tracking-wider rounded-apple hover:shadow-sm transition-all flex/g, 'bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-black text-[10px] uppercase tracking-widest rounded-apple shadow-emerald-sm transition-all flex');

// 5. Check circles
content = content.replace(/text-emerald-600/g, 'text-emerald-500'); 
// wait, the default was 400 that I made 600. Some places had it. It's fine.
